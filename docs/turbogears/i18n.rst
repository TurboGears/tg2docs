...
If the quickstarted project includes a catalog for your browser language,
its generated welcome page can be displayed in that language.
...
1. the language saved in the session by :func:`tg.i18n.set_lang`,
2. the browser's ``Accept-Language`` values,
3. ``i18n.lang``, if configured.
...
using the :func:`tg.i18n.set_request_lang` function. This changes
...
application code calls :func:`tg.i18n.set_request_lang` or
:func:`tg.i18n.set_lang` during the request.

I18N Configuration
------------------

For a complete reference of all available i18n configuration options, see
:ref:`Internationalization <config-options>`.
...
``i18n.enabled``
    Enables the i18n request wrapper that reads the session language and the
    browser's ``Accept-Language`` header. Set it to ``false`` to disable
    automatic language detection.

``i18n.native``
    Space-separated list, or Python list, of languages the templates are
...
``i18n.lang_session_key``
    Session key read by the i18n request wrapper. The default is ``tg_lang``.
...
``i18n.no_session_touch``
    Avoids marking a previously untouched session as accessed just because the
    i18n wrapper checked it for a saved language. The default is ``false``.

``localedir``
    Directory containing runtime gettext catalogs. It defaults to
...
``i18n.domain``, ``i18n.locale_dir``, and ``i18n.gettext_fallback``
    These are not TurboGears runtime i18n options. Runtime translations use the
...
Don't forget to add the generated catalog files to your revision control system
if you use one.
...
5. Configure the fallback language in ``development.ini`` if you want one to
   be used when the browser or session does not select a supported language::

    [app:main]
    i18n.lang = zh_tw

   To force that language for the whole application instead of using browser
   language negotiation, disable automatic detection too::
...
Update an existing catalog after extracting new messages with the following
command::
...
.. _UserGuide: https://babel.pocoo.org/en/latest/
