...
When authentication succeeds, TurboGears uses the ``TGAuthMetadata`` instance
configured with the ``sa_auth.authmetadata`` key in
``base_config.update_blueprint(...)`` to retrieve the information about your
user which is required for authorization.
...
simply use a mix of patterns. To do this you can set ``sa_auth.authmetadata`` to
``None`` with ``base_config.update_blueprint(...)`` and register your own
metadata providers for :mod:`repoze.who`.
...
"predicate checkers". The predicate checkers live in TurboGears itself and can
be imported directly from :mod:`tg.predicates`.
...
your controller class::
    ...
    from tg import expose, flash, predicates
...
``allow_only`` attribute::
    ...
    from tg import expose, predicates, require
    from yourproject.lib.base import BaseController
...
But you can also nest compound predicates::
    ...
    from tg.predicates import All, Any, has_permission
    from yourproject.lib.auth import is_month
    # ...
    @require(All(
        Any(is_month(4), is_month(10)),
        has_permission('release')
    ))
